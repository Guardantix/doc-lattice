...
```bash
uvx --python 3.13 --from doc-lattice==7.1.0 doc-lattice init --default-branch main
```
    ...
    steps:
      ...
      - name: Install pinned doc-lattice without the Linear secret
        run: |
          ...
          uv pip install --python "$RUNNER_TEMP/doc-lattice-venv/bin/python" doc-lattice==7.1.0
...
- **A Linear key can be checked, but the check is about your copy rather than GitHub's.** `--from`
  ...
  ```bash
  uvx --python 3.13 --from doc-lattice==7.1.0 doc-lattice linear --from SOME_UPSTREAM_ID
  ```
  ...
  uv pip install --python "$venv/bin/python" doc-lattice==7.1.0
  ```
