...
expands the ``link_sources`` selectors through ``path_selection``'s shared no-follow walk.
``check_links`` reads selected documents and every repository-contained target they link to. Both
return data rather than prose: findings are ``LinkFinding`` records, and the
``path[:line]: message`` envelope belongs to the command adapter.
