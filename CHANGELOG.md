...
- `check`, `lint`, `impact`, and `linear` accept `--indent N` with `--json`, and the global
  `--no-color` option explicitly disables colored output, including the styling on help and
  usage-error text even when a terminal-forcing environment variable is set (#20).
